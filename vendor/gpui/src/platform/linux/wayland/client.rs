...
pub(crate) struct WaylandClientState {
    ...
    clipboard: Clipboard,
    dragged_file_uris: Option<Vec<u8>>,
...
impl WaylandClientStatePtr {
    ...
    pub fn start_file_drag(
        ...
        // Marcel extension to GPUI 0.2.2's Wayland data-device implementation.
        // Upstream source: https://github.com/zed-industries/zed/tree/main/crates/gpui
        if paths.is_empty() {
            return false;
        }
        ...
        state.dragged_file_uris = Some(uri_list.into_bytes());
    ...
    fn event(
        ...
        match event {
            wl_data_source::Event::Send { mime_type, fd } => {
                if mime_type == FILE_LIST_MIME_TYPE {
                    if let Some(uri_list) = state.dragged_file_uris.clone() {
                        state.clipboard.send_file_list(fd, uri_list);
                    }
                } else {
