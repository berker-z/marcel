...
impl Window {
    ...
    /// Starts a native copy drag containing local files.
    ///
    /// Returns `false` when the active platform does not support native file
    /// drag sources.
    pub fn start_file_drag(&self, paths: Vec<std::path::PathBuf>) -> bool {
